...
/**
 * Wraps an API route handler and requires a valid session AND one of the
 * given roles (defaults to admin-only). Use for user management, settings, etc.
 */
export function requireRole(handler, allowedRoles = ["admin"]) {
...
export const requireAdmin = (handler) => requireRole(handler, ["admin"]);

// --- Temporary token used between "password verified" and "2FA verified"
// during login. Short-lived (5 min), separate secret purpose from the main
// session token so it can never be mistaken for a full session cookie.
export function signTempLoginToken(payload) {
  return jwt.sign({ ...payload, purpose: "2fa-pending" }, JWT_SECRET, { expiresIn: "5m" });
}

export function verifyTempLoginToken(token) {
  const decoded = verifyAuthToken(token);
  if (!decoded || decoded.purpose !== "2fa-pending") return null;
  return decoded;
}
